...
Because the optimization problem is convex, it can be shown that this type of descent algorithm is guaranteed to converge to the optimal solution. That is, it does not get `stuck' at any inflection points or local minima. Therefore, if that was all the algorithm did, there would be no need to check the KKT conditions for optimality. 
...
The motivation for the strong rule comes from the KKT stationarity condition, which is similar to the first derivative test for minima \citep{boyd2004convex}. Explicitly, this is the condition that the subdifferential equations (see \autoref{eq:subgrad}) are satisfied at the proposed minimum $\hat{\beta}^{(k)}(\lambda_s)$. Note that we are still focusing on a given group $k$, but we are emphasizing the dependence on $\lambda$ now.
 
From \autoref{eq:subgrad} we derive the condition used in the algorithm to check the KKT stationarity condition. Because the algorithm is guaranteed to converge to the optimum for all active groups, we only need to check the condition required for $\hat{\beta}^{(k)} = 0$ to be the solution (at a given $\lambda$). First, the subgradient equations reduce to the following:

\[
X^{(k)T}r_{(-k)}/n - \alpha \lambda v = (1-\alpha) \lambda u,
\]
where $u$ and $v$ are the subdifferentials of the $l-2$ and $l-1$ norms, as in section 2.1. We claim that if the following inequality

\[
||S(X^{(k)T}r_{(-k)}/n,\ \alpha\lambda)||_2 \leq (1-\alpha)\lambda.
\]
is satisfied (where $S$ is the soft-threshold operator), then $\hat{\beta}^{(k)}=0$ is the minimum for that group at that particular lambda. The reason for this is that we can find a subgradient $v$ such that $X^{(k)T}r_{(-k)}/n - \alpha \lambda v = S(X^{(k)T}r_{(-k)}/n,\ \alpha\lambda)$, and if the above inequality is satisfied, we can also find a subgradient $u$ such that the subgradient equations are satisfied.
