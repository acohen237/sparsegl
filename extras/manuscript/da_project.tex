...
\]
A variant of this, the group lasso \citep{yuan2006model} is appropriate when there is a naturally grouped structure for the coefficients $\beta = (\beta^{(1)},\ \beta^{(2)},\ \dots, \ \beta^{(m)})$ ; it performs regularization that has the effect of selecting for the groups of predictors rather than the predictors themselves. It is of the form 
\[
...
\begin{equation}
\ell(\beta^*)+(\beta - \beta^*)^T\nabla \ell(\beta^*)+\frac{t}{2}\norm{\beta^*-\beta}_2^2+ (1-\alpha)\lambda\sum_{l=1}^m\sqrt{p_l}||\beta^{(l)}||_2+\alpha\lambda||\beta||_1,
\end{equation}
which can be solved. The resulting update step that $\beta^{(k)}$ is replaced by \djm{need to display what the GD process looks like}
\begin{equation}
\beta^{(k)} \leftarrow\left(1-\frac{t(1-\alpha)\lambda}{||S(\beta^{(k)}-t\nabla \ell(r_{-k)},\beta^{(k)}),\ t\alpha\lambda)||_2}\right)_+ S(\beta^{(k)}-t\nabla \ell(r_{(-k)},\beta^{(k)}),t\alpha\lambda)
\label{eq:updateStep}
\end{equation}
...
The motivation for the strong rule comes from the KKT check, so we start with this. The KKT stationarity condition \citep{karush2014minima} is essentially the first-derivative test from calculus, and for this problem is 
\[
...
\begin{algorithm}
  ...
  \begin{algorithmic}
    \STATE {\bf Input:} $\Lambda = \{\lambda_0>\lambda_1> \dots$\},\ $\mathcal{E}$,\ $\mathcal{A}$
    \STATE Set $\mathcal{E}=\mathcal{A}=\emptyset$ 
    	\COMMENT{Step 0}
    \STATE {\bf Step 1:} $\mathcal{E} \leftarrow  \mathcal{E}\cup \mathcal{S}(\lambda,\ \mathcal{E}^c)$
    \WHILE{MAXDIF $> \epsilon$} 
	\FOR{$k=1$ to $numgroups$}    
    \STATE  $\beta^{(k)} \leftarrow \Gamma (\beta^{(k)})$
    \STATE If $\beta^{(k)} \neq 0$ put $\beta^{(k)} \in \mathcal{A}$
	\ENDFOR 
    \STATE Run KKT check on $\mathcal{E}^c$, put violators in $\mathcal{E}$; if there are violators, go back to step 2, otherwise move on.
